...
void setup() {
  ...
  border = loadImage('./assets/border.gif');
  ballAimer = loadImage('./assets/ballAimer.gif');
...
void drawBoard() {
  image(bg, 0, 0, 670, canvasHeight);
  image(border, 100, -100, 435, canvasHeight + 190);
...
void draw() {
  ...
  for (var i = 0; i < bubbles.length; i++) {
    ...
    if (bubbles[i].color === 'blue') {
      image(blue_bubble, bubbles[i].x - 25, bubbles[i].y - 25, bubbleRadius * 2, bubbleRadius * 2);
    } else if (bubbles[i].color === 'red') {
      image(red_bubble, bubbles[i].x - 25, bubbles[i].y - 25, bubbleRadius * 2, bubbleRadius * 2);
    } else {
      image(yellow_bubble, bubbles[i].x - 25, bubbles[i].y - 25, bubbleRadius * 2, bubbleRadius * 2);
    }
  ...
  if (!lastBub.active) {
    ...
    Bubble bubble = new Bubble(xStart, yStart - 25, level.bubbleColors[c]);
  }
...
void renderCannon() {
  pushMatrix();
  translate(335, 450);
  rotate(cannonDirection/15);
  image(ballAimer, -25, -70, 50, 120);
  popMatrix();
}
